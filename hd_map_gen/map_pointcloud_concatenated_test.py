...
def project_points_on_map(points, frame):
    """
    Transform points from the vehicle frame to the global frame
    """
    transform = np.reshape(np.array(frame.pose.transform), [4, 4])
    points_homogeneous = np.hstack((points, np.ones((points.shape[0], 1))))
    global_points = np.dot(transform, points_homogeneous.T).T

    return global_points[:, :3]  # Return only X, Y, Z

def project_points_on_vehicle(points_global, frame):
    """
    Transforms a point cloud from the global frame to the vehicle frame.
    """
    transform = np.reshape(np.array(frame.pose.transform), [4, 4])
    # Compute the inverse of the pose transformation matrix
    pose_inverse = np.linalg.inv(transform)
    points_homogeneous = np.hstack((points_global, np.ones((points_global.shape[0], 1))))
    vehicle_points_homogeneous = np.dot(pose_inverse, points_homogeneous.T).T

    # Return only the X, Y, Z coordinates
    return vehicle_points_homogeneous[:, :3]
...
if __name__ == "__main__":
    ...
    for scene_index, scene_path in enumerate(sorted(tfrecord_list)):
        ...
        for frame in load_frame(scene_path):
            ...
            point_labels_ri1 = convert_range_image_to_point_cloud_labels(
                frame, range_images, segmentation_labels)
            point_labels_ri2 = convert_range_image_to_point_cloud_labels(
                frame, range_images, segmentation_labels, ri_index=1)
            # Concatenate both labels returs
            if isinstance(point_labels_ri1, list) and isinstance(point_labels_ri2, list):
                point_labels = [np.concatenate([l1, l2], axis=0) for l1, l2 in zip(point_labels_ri1, point_labels_ri2)]
            else:
                # If they are single arrays, concatenate directly
                point_labels = np.concatenate([point_labels_ri1, point_labels_ri2], axis=0)

            # Concatenate labels of points of the 5 LiDAR
            concat_point_labels = concatenate_points(point_labels)
            filtered_point_cloud, filtered_point_labels = filter_lidar_data(points, concat_point_labels, [8, 10])

            # Project pointcloud to global frame > this operation is done to be able to concatenate pointclouds of every frame with t
            filtered_point_cloud = project_points_on_map(filtered_point_cloud, frame)

            # Get semantic and instance segmentation labels
            semantic_labels = filtered_point_labels[:,1]
            instance_labels = filtered_point_labels[:,0]
        ...
        # Convert points back from global frame to vehicle frame
        point_clouds_vehicle = project_points_on_vehicle(point_clouds, frame)
        plot_referenced_pointcloud(point_clouds_vehicle, True)

        # Ensure points and labels match
        if point_clouds.shape[0] != point_cloud_labels.shape[0]:
        ...
        clustered_point_cloud, cluster_labels = cluster_pointcloud(point_clouds_vehicle)
